...
def login():
    if request.method == "POST":
        ...
        exist_user = User.query.filter_by(name = user).first()
...
def admin():
    ...
    return render_template("admin/index.html", user=user,param=param, all=all_post)
...
def edit(post_id):
    if request.method == "GET":
        ...
        return render_template("admin/edit_post.html", param=param, post=post1)
    else:
        if post_id != 0:
